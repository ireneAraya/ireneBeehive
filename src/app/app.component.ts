...
export class AppComponent {
  ...
  loadData(usersUrl: string, postsUrl: string, albumsUrl: string, photosUrl: string, todosUrl: string, commentsUrl: string) {
    this.http.get(usersUrl).map(res => res.json()).subscribe((data) => {
      ...
      let company = new Company('Foto&Gráfica', 'A better world for everyone', 'Dream does not cost a dine');
      let address = new Address('San Antonio', 'Goicoechea', 'Guadalupe', '1000', new Geo
        ('0.000001', '0.000002'));
